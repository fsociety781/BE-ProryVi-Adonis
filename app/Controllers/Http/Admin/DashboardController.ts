...
export default class DashboardController {
  ...
  try{
    ...
    const lastRequest = await Database.from('items')
    ...
    const totalCount = await Database.from('items').count('* as total')
    const itemsOnproccesResult = await Database.from('items').where('status', 'Onprocces').andWhereBetween('created_at', [startDate, endDate]).count('* as total')
    const itemsApproveResult = await Database.from('items').where('status', 'Approve').andWhereBetween('created_at', [startDate, endDate]).count('* as total')
    const itemsRejectResult = await Database.from('items').where('status', 'Reject').andWhereBetween('created_at', [startDate, endDate]).count('* as total')

    const allOnproccesResult = await Database.from('items').where('status', 'Onprocces').count('* as total').first();
    const allApproveResult = await Database.from('items').where('status', 'Approve').count('* as total').first();
    const allRejectResult = await Database.from('items').where('status', 'Reject').count('* as total').first();
